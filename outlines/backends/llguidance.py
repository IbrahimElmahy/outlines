...
class LLGuidanceBackend(BaseBackend):
    ...
    def __init__(self, model: SteerableModel):
        ...
        tokenizer_wrapper = getattr(model, "tokenizer", None)
        if tokenizer_wrapper is not None and hasattr(tokenizer_wrapper, "special_tokens"):
            self._special_tokens = set(tokenizer_wrapper.special_tokens)
        else:
    ...
    def get_cfg_logits_processor(
        ...
        processed_ebnf = preprocess_cfg_grammar(
            grammar,
            special_tokens=self._special_tokens,
            grammar_format="ebnf",
        )
        try:
            grammar_spec = self.llg.grammar_from("grammar", processed_ebnf)
        except ValueError:
            processed_lark = preprocess_cfg_grammar(
                grammar,
                special_tokens=self._special_tokens,
                grammar_format="lark",
            )
            grammar_spec = self.llg.grammar_from("lark", processed_lark)
        return LLGuidanceLogitsProcessor(
            grammar_spec, self.llg_tokenizer, self.tensor_library_name
        )
